# ---------------------------------------------------------------------------------------------------------------------
# Aviatrix AWS Security VPC | dev-ext-fw
# ---------------------------------------------------------------------------------------------------------------------
resource "aviatrix_vpc" "dev_ext_fw_vpc" {
  cloud_type           = 1
...
# ---------------------------------------------------------------------------------------------------------------------
# Aviatrix Transit Firenet Gateway | dev-ext-fw-gw
# ---------------------------------------------------------------------------------------------------------------------
resource "aviatrix_transit_gateway" "dev_ext_fw_gw" {
  ...
  gw_size      = "c5.xlarge"
  subnet       = aviatrix_vpc.dev_ext_fw_vpc.public_subnets[0].cidr
  ...
  depends_on = [aviatrix_vpc.dev_ext_fw_vpc]
}
...
# Launch Firewall
# ---------------------------------------------------------------------------------------------------------------------
resource "aviatrix_firewall_instance" "dev_ext_fw_instance" {
  vpc_id          = aviatrix_vpc.dev_ext_fw_vpc.vpc_id
  firenet_gw_name = aviatrix_transit_gateway.dev_ext_fw_gw.gw_name
  firewall_name   = "dev-ext-fw-instance-1"
  firewall_image  = "Fortinet FortiGate Next-Generation Firewall"
  firewall_size   = "t2.small"
  egress_subnet   = aviatrix_vpc.dev_ext_fw_vpc.subnets[1].cidr
  ...
  #user_data  = local.dev_ext_fw_init_conf
...
resource "aviatrix_firewall_instance_association" "dev_ext_fw_instance_assoc" {
